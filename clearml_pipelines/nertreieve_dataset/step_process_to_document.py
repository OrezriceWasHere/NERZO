...
def process_document(document):
        ...
        for tagging_instance, instance_content in type_content.items():
            for phrase, references in instance_content.items():
                for reference in references:
                    ...
                    index_start = sum(len(word) for word in document['document_token_sequence'][:word_index_start]) + word_index_start
                    index_end = index_start + len(" ".join(document['document_token_sequence'][min(reference):max(reference) + 1] ))
    ...
    if not tagging_array:
        print("No tagging entities found. line: " + json.dumps(document))
        return None
