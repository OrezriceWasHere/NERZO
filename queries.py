...
def query_get_by_fine_grained_fewnerd_v3_randomized(
		fine_grained_type: str | list[str], batch_size,
		llm_layer=None, entity_type_key="fine_type"
		) -> dict:
	fine_grained_type = fine_grained_type if isinstance(fine_grained_type, list) else [fine_grained_type]
	query = {
		"query": {
				...
				"query": {
					"bool": {
						"filter": [
							{
								"terms": {
									entity_type_key: fine_grained_type
								}
							}
						]
					}
...
def query_get_by_fine_grained_fewnerd_v3(
		fine_grained_type: str | list[str], randomized=True,
		batch_size: int = 200, llm_layer=None
		) -> dict:
	fine_grained_type = fine_grained_type if isinstance(fine_grained_type, list) else [fine_grained_type]
	if randomized:
...
def query_hard_negative(
		fine_grained_type: str | list[str],
	...
	assert isinstance(fine_grained_type, list) or isinstance(
		fine_grained_type,
		str
		), "fine_grained_type should be a string or a list of strings"
