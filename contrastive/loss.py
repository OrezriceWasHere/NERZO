...
class ContrastiveLoss(torch.nn.Module):
    ...
    def __init__(self, loss_fn="info_nce", **kwargs):
...
def info_nce(query, positive_key, negative_keys=None, temperature=0.1, reduction='mean', negative_mode='unpaired'):
    ...
    #
    # mone = torch.exp(positive_logit / temperature)
    # mechane = torch.sum(torch.exp(negative_logits / temperature), dim=1, keepdim=True)
    # loss = -torch.log(mone / mechane)
