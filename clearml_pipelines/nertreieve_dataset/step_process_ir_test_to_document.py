...
def process_batch(batch):
	...
	for document in batch:
		tagging_array = []
		text_id = document["id"]
		all_text = text_id_to_content[text_id]["content"]
			...
			for tagging_instance, instance_content in type_content.items():
				for phrase, references in instance_content.items():
					for reference in references:
						...
						document_token_sequence = all_text.split(" ")
						index_start = sum(
							len(word) for word in document_token_sequence[:word_index_start]
						) + word_index_start
...
def load_corpus():
	...
	dataset_file = StorageManager.get_local_copy(remote_url=corpus_url)
	print("unzipping dataset...")
	source_file = dataset_file
	uncompressed_file = dataset_file[:-4]
	with open(source_file, 'rb') as source, open(uncompressed_file, 'w') as dest:
	...
	with open(uncompressed_file) as file:
		for line in file:
			line_json = json.loads(line)
			text_id = line_json.pop("id")
			result[text_id] = line_json
	return result
...
if __name__ == "__main__":
	...
	text_id_to_content = load_corpus()
