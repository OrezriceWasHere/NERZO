...
def get_sentences_and_entities(fine_entity: str):
	text_id_sentences_with_two_entities_query = {
		"size": 0,
		"aggs": {
			"texts_with_multiple_tags": {
				"terms": {
					"field": "text_id",
					"min_doc_count": 2,
					"size": 10000
	...
	sentences_with_entity = dataset_provider.search(text_id_sentences_with_two_entities_query, index="fewnerd_v4_*")
	text_id_sentences_with_two_entities = [item["key"] for item in
	                                       sentences_with_entity["aggregations"]["texts_with_multiple_tags"][
		                                       "buckets"]][-15:]
	...
	query = {
		"query": {
			"function_score": {
				"query": {
					"bool": {
						"filter": [
							{"terms": {"text_id": text_id_sentences_with_two_entities}}
						]
					}
				},
		...
		"size": 10000,
		"_source": ["all_text", "doc_id", "phrase", "index_start", "index_end", "fine_type"]
	}
	docs = dataset_provider.search(query=query, index="fewnerd_v4_*")["hits"]["hits"]

	for result in docs:
		doc = result["_source"]
		entity_info = {
			"text": doc["all_text"],
			"index_start": doc["index_start"],
			"index_end": doc["index_end"],
			"phrase": doc["phrase"].strip(),
			"id": doc["doc_id"]
		}
		if doc["fine_type"] == fine_entity:
...
def compute_word_similarity(entity, match):
	...
	start1, end1 = llm.token_indices_given_text_indices(match["text"], (match["index_start"], match["index_end"]))
...
def find_threshold_hooked_epoch(pairs, different_type, epoch):
	...
	for group_id, group in group_to_item.items():
		...
		group_accuracy = np.sum(
			((x[group_indices] >= optimal_threshold_seperator[group_indices]) == y[group_indices]),
			axis=1
		) / x.shape[1]
		clearml_poc.add_scatter(
...
def main():
	clearml_poc.clearml_init(requirements=["protobuf", "sentencepiece"])
