...
@PipelineDecorator.component(return_values=["embedding_json"], task_type=TaskTypes.data_processing,
                             execution_queue="a100_gpu"
                             )
def step_create_embedding(llm_id: str,
                          layer: str,
...
def step_index_to_elastic(embeddings_json:list[dict]):
    ...
    loop.run_until_complete(dataset_provider.ensure_existing_index(index, fewnerd_dataset.elasticsearch_fine_type_to_embedding))
    ...
    index_to_elastic_tasks = [dataset_provider.upsert(data=embedding, index=index, doc_id=embedding["entity_id"]) for embedding in embeddings_json]
...
@PipelineDecorator.pipeline(name="Forward fine type names in Elasticsearch", project="fewnerd_pipeline", version="0.0.1",
                            packages="./requirements.txt",
                            pipeline_execution_queue="a100_gpu"
                            )
def executing_pipeline(llm_id: str,
                       layer: str,
    ...
    embeddings = step_create_embedding(llm_id,
                          layer,
                          max_llm_layer)
